...
class lstm_model():
    ...
    def __init__(self, vocab_size, embedding_size, words_in_sentence, batch_size, lstm_cell_size): # sequence_length, filter_sizes, num_filters, l2_reg_lambda=0.0
        ...
        self.init_state_hidden = tf.placeholder(tf.float32, [batch_size, lstm_cell_size], name='init_state_hidden') # [batch_size, num_steps]
        self.init_state_current= tf.placeholder(tf.float32, [batch_size, lstm_cell_size], name='init_state_current') # [batch_size, num_steps]
        ...
        with tf.device('/cpu:0'):
            ...
            with tf.variable_scope("embedding_layer"):
                W_embedding = tf.Variable(tf.random_uniform([vocab_size, embedding_size], -0.1, 0.1), name="W_embedding") #[vocab_size, embedded_word_size]

                W_embedding = tf.get_variable(name="W_embedding", initializer=tf.random_uniform([vocab_size, embedding_size], -1.0, 1.0)) # [vocabulary_size, word_embedding_size]
                embedded_input = tf.nn.embedding_lookup(W_embedding, self.input_x) # [batch_size, num_steps, word_embedding_size]
            ...
            self.predictions_per_sentence = tf.reshape(history_predictions, [batch_size * words_in_sentence, lstm_cell_size])
            ...
            with tf.variable_scope("softmax_out"):
                ...
                W_soft = tf.Variable(tf.random_uniform([lstm_cell_size, vocab_size], -0.1, 0.1), name="W_soft") #[vocab_size, embedded_word_size]
                b_soft = tf.Variable(tf.random_uniform([vocab_size], -0.1, 0.1), name="b_soft") #[vocab_size]
                #W_soft = tf.Variable("W", [lstm_cell_size, vocab_size], tf.float32, initializer=tf.contrib.layers.xavier_initializer())
                #W_soft = tf.get_variable("W_soft",[lstm_cell_size, vocab_size], tf.float32, initializer=tf.contrib.layers.xavier_initializer())
                ...
                self.logits = tf.matmul(self.predictions_per_sentence, W_soft) + b_soft # [total_nb_of_words_in_batch, predictions_on_vocabulary]
            ...
            with tf.name_scope("loss"):
                ...
                all_losses = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logits, labels=self.vectorized_groundtruth)
